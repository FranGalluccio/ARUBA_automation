...
def test_disponibilita_libero(page):
    ...
    try:
        ...
        page.screenshot(path=os.path.join(REPORT_FOLDER, f"test_calendario_08_invitato_{datetime.now():%H-%M-%S}.png"))
        ...
        page.screenshot(path=os.path.join(REPORT_FOLDER, f"test_calendario_08___{datetime.now():%Y-%m-%d_%H-%M-%S}.png"), full_page=True)
        assert not occupato_visibile, \
            "Slot risulta 'Occupato' anche con 'Mostrati come occupato' disattivato — comportamento errato"
        print("test_calendario_08 PASSED — slot non occupato con 'Mostrati come occupato' OFF")
