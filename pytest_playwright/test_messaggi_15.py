...
def test_sposta_messaggio_in_cartella(page):
    # Login PEC
    ...
    Helper.crea_messaggio(
        page,
        config,
        oggetto=f"Test sposta playwright {ts}",
        corpo="Questo messaggio verrà spostato in una cartella",
    )
    page.locator('span[title="Invia"], span[title="Envoyer"]').click(force=True)
    ...
    screenshot_path = os.path.join(
        REPORT_FOLDER,
        f"test_messaggi_15___{datetime.now():%Y-%m-%d_%H-%M-%S}.png"
    )
